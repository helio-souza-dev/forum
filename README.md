...
**
|
 React 18, Vite 5, lucide-react, Canvas WebP Cropper 
|
|
**
Backend
**
|
 Node.js, Express 4, Multer (Upload Blindado), CORS 
|
|
**
Segurança & Auth
...
|
 Catbox.moe API (
`Upload grátis de avatares`
) + fallback de disco local (
`/uploads/`
) 
|
|
...
`/api/media`
|
 Lista mídias do feed local (filtros por tag, tipo ou busca) 
|
 Não 
|
|
...
`/api/media/:id`
|
 Carrega detalhes de uma mídia e incrementa visualizações 
|
 Não 
|
|
...
`/api/media`
|
 Publica nova mídia no feed (inspeção de Magic Bytes ativa) 
|
 Não / Opcional 
|
|
...
`/api/upload/free`
|
 Upload 100% grátis de avatares/banners para Catbox.moe ou disco 
|
 Não 
|
|
...
`/api/users/:username`
|
 Retorna dados públicos, bio e lista de posts de um perfil 
|
 Não 
|
|
...
`/api/users/profile`
|
 Atualiza avatar, banner, bio e configurações do perfil 
|
**
Sim (JWT)
**
...
`/api/booru/search`
|
 Consulta externa em Boorus com identificação de @Autor 
|
 Não 
|
|
...
`/api/booru/import`
|
 Importa post externo para o banco de dados local 
|
**
Sim (JWT)
**
...
`/api/hotlink`
|
 Proxy com proteção anti-SSRF e suporte a Range Requests 
|
 Não 
|
|
...
`/api/auth/register`
|
 Registro com verificação de senha segura (mín. 6 caracteres) 
|
 Não (
`Rate Limiter`
) 
...
`/api/auth/login`
|
 Autenticação do usuário e emissão de Token JWT 
|
 Não (
`Rate Limiter`
) 
...
`/api/admin/*`
|
 Moderação de mídias e gestão de contas 
|
**
Sim (
`admin`
...
`/api/dev/stats`
|
 Telemetria de infraestrutura e log de auditoria do sistema 
|
**
Sim (
`dev`
